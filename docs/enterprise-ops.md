...
`f12042a`; upstream PR #391 adds the holder-dialed push engine, while votport
...
| Notify | `server/src/notify.rs`: webhook, ntfy, Pushover from `app.config`. Fire-and-forget. `upload.rs` already `tokio::spawn`s `notify::uploaded`; `app::upload_ended_notifier` sends `upload_failed` (outcome `rejected` or `interrupted`, never `cancelled`) for links with notify on upload set. |
