...
- Gates: rustup 1.97.1, `cargo fmt --check`, `cargo clippy --all-targets -- -D warnings`, `cargo test` (40 unit + 12 e2e today), `node --check` + eslint on `web/assets/*.js`, `node --test scripts/*.test.mjs`, cargo-deny advisories, docker build.
