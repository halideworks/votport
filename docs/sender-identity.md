...
`vot-receipt` at pin `296174a` exposes `SubjectKind` / `AssuranceLevel` / `CommitProfile` as `repr(u8)` with no serde aliases and no `Display`. Do not `format!("{:?}", …)` (`Object` / `Published` / `Balanced`). Explicit match, and tests for the strings:
...
- vot-receipt at pin `296174a794c58352b08c622d6ccdda5cb73122f2`: `decode_authenticated`, `verify_ed25519`, 65_536 byte cap
