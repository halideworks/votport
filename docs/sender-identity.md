...
`vot-receipt` at pin `de66d413` exposes `SubjectKind` / `AssuranceLevel` / `CommitProfile` as `repr(u8)` with no serde aliases and no `Display`. Do not `format!("{:?}", …)` (`Object` / `Published` / `Balanced`). Explicit match, and tests for the strings:
...
- vot-receipt at pin `de66d41343cee7fb206a0f323d36babea655f032`: `decode_authenticated`, `verify_ed25519`, 65_536 byte cap
