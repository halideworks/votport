...
`vot-receipt` at pin `b14cc41` exposes `SubjectKind` / `AssuranceLevel` / `CommitProfile` as `repr(u8)` with no serde aliases and no `Display`. Do not `format!("{:?}", …)` (`Object` / `Published` / `Balanced`). Explicit match, and tests for the strings:
...
- vot-receipt at pin `b14cc41debc2547c5ef999fee26bb055995284d9`: `decode_authenticated`, `verify_ed25519`, 65_536 byte cap
