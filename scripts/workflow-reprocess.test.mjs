...
test('reprocess is offered only for inactive prepared receptions under changed rules', () => {
  const { action } = setup();
  for (const state of ['failed', 'retrying', 'awaiting_approval', 'ready']) assert.ok(action({ ...job, state }, project));
  for (const state of ['queued', 'preparing', 'exporting', 'cancelled', 'retiring', 'retired', 'suspended']) assert.equal(action({ ...job, state }, project), null);
  for (const change of [{ received: null }, { manifest: null }, { reprocessed_as: 'new' }]) assert.equal(action({ ...job, ...change }, project), null);
  assert.equal(action(job, { ...project, receive: false }), null);
  assert.equal(action(job, { ...project, revision: 1 }), null);
  assert.equal(action(job, undefined), null);
  assert.equal(setup({ session: { subject: 'observer' } }).action(job, project), null);
  assert.ok(setup({ admin: true, session: { subject: 'administrator' } }).action(job, project));
});
...
test('reprocess confirms new copies and binds the refreshed revision and frozen manifest', async () => {
  ...
  await state.action(job, project).click();
  ...
  await cancelled.action(job, project).click();
