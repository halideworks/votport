...
import { ssoErrorMessage } from '../web/assets/login-errors.js';

test('SSO codes preserve actionable messages', () => {
  ...
  ]) assert.equal(ssoErrorMessage(code), expected);
});

test('unrecognized SSO codes cannot supply banner text', () => {
  for (const code of [
    'failed', 'Contact attacker.invalid', Buffer.from('Contact attacker.invalid').toString('hex'),
    'constructor', '__proto__', 'toString', 'ACCOUNT_BLOCKED', '', null, undefined,
  ]) assert.equal(ssoErrorMessage(code), 'SSO sign-in failed. Try again or contact your administrator.');
});
