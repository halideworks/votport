...
| `VOTPORT_REPLICA_TOKEN` | — | Bearer a standby presents to `GET /api/replica`; unset disables it. Overridable via `PUT /api/admin/settings`. |
| `VOTPORT_STANDBY_SOURCE` | — | `votport standby` only: the live instance's https URL to pull replicas from. |
