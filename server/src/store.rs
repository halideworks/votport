...
fn outbound_grant_previews(
...
) -> rusqlite::Result<Vec<(OutboundGrant, usize)>> {
    ...
    let file_preview_limit =
        i64::try_from(file_preview_limit).map_err(|_| rusqlite::Error::InvalidQuery)?;
    ...
    let mut statement = connection.prepare(&format!(
                    "SELECT id, token_hash, password_hash, tenant, link_id, upload_id, package_root,
                     ...
                     FROM outbound_grants WHERE tenant = ?1 {filter}
                     ORDER BY created_at DESC, rowid DESC LIMIT ?2 OFFSET ?3",
                ))?;
    let rows = statement.query_map(
        rusqlite::params![tenant, limit, offset, file_preview_limit, id],
